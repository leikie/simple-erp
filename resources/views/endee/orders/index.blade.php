	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
		...
		<div class="btn-toolbar mb-2 mb-md-0">
			<a href="{{ route('orders.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
				<svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> New Order
			</a>
			...
			<div class="card card-table show-entire">
				<div class="card-body p-3">
					<div class="input-group mb-5">
						...
						<input type="text" id="my_search" class="form-control" placeholder="Search orders">
					</div>
					...
					<div class="table-responsive">
						<table id="datatable-order" class="table user-table table-hover align-items-center">
...
@section('scripts')		
	...
	<script>
        ...
        function datatables_orders() {
            dataOrders = $('#datatable-order').DataTable({
                'destroy': true, 
				"dom": "lrtip",
                'order': [
                    [2, 'desc'],
            ...
            dataOrders.on('order.dt search.dt', function() {
                dataOrders.column(1, {
                    search: 'applied',
                    order: 'applied'
		...
		$('#datatable-order').on('click', '.delete-order', function(e) {
			e.preventDefault() // Don't post the form, unless confirmed
			let order_id = $(this).data('order')
			$('#delete_order').modal('show')
			$('#delete_order form').attr('action', "{{ url('/orders') }}/"+order_id)
    	});

		$('#my_search').keyup(function() {
      		dataOrders.search($(this).val()).draw()
		})
	</script>
@endsection
