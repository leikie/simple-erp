...
@section('content')	
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
		<div class="d-block mb-4 mb-md-0">
			...
			<p class="mb-0">users have roles</p>
		</div>

		<div class="btn-toolbar mb-2 mb-md-0">
			<a href="{{ route('roles.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
				<svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> New Role
			</a>
			...
			<div class="card card-table show-entire">
				<div class="card-body p-3">
					<div class="input-group mb-5">
						...
						<input type="text" id="my_search" class="form-control" placeholder="Search roles">
					</div>
					...
					<div class="table-responsive">
						<table id="datatable-role" class="table user-table table-hover align-items-center">
							<thead>
								<tr>
									<th>
...
@section('scripts')	
	...
	<script>
        ...
        function datatables_role() {
            dataRole = $('#datatable-role').DataTable({
                'destroy': true, 
				"dom": "lrtip",
                'order': [
                    [2, 'desc'],
            ...
            dataRole.on('order.dt search.dt', function() {
                dataRole.column(1, {
                    search: 'applied',
                    order: 'applied'
		...
		datatables_role()
		$('#datatable-role').on('click', '.delete-role', function(e) {
			e.preventDefault() // Don't post the form, unless confirmed
			let role_id = $(this).data('role')
			$('#delete_role').modal('show')
			$('#delete_role form').attr('action', "{{ url('/roles') }}/"+role_id)
    	});

		$('#my_search').keyup(function() {
      		dataRole.search($(this).val()).draw()
		})
	</script>
@endsection
