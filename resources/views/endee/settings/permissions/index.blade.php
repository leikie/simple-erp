...
@section('content')	
	...
	<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
		<div class="d-block mb-4 mb-md-0">
			...
			<p class="mb-0">roles have permissions</p>
		</div>

		<div class="btn-toolbar mb-2 mb-md-0">
			<a href="{{ route('permissions.create') }}" class="btn btn-sm btn-gray-800 d-inline-flex align-items-center">
				<svg class="icon icon-xs me-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg> New Permission
			</a>
			...
			<div class="card card-table show-entire">
				<div class="card-body p-3">
					...
					<div class="input-group mb-5">
						...
						<input type="text" id="my_search" class="form-control" placeholder="Search permissions">
					</div>
					...
					<div class="table-responsive">
						<table id="datatable-permission" class="table user-table table-hover align-items-center">
							<thead>
								<tr>
									<th>
	...
	<div id="delete_permission" class="modal fade delete-modal" role="dialog">
...
@section('scripts')		
	...
	<script>
        ...
        function datatables_permission() {
            dataPermission = $('#datatable-permission').DataTable({
                'destroy': true,
				"dom": "lrtip", 
                'order': [
                    [2, 'desc'],
            ...
            dataPermission.on('order.dt search.dt', function() {
                dataPermission.column(1, {
                    search: 'applied',
                    order: 'applied'
		...
		datatables_permission()
		$('#datatable-permission').on('click', '.delete-permission', function(e) {
			e.preventDefault() // Don't post the form, unless confirmed
			let permission_id = $(this).data('permission')
			$('#delete_permission').modal('show')
			$('#delete_permission form').attr('action', "{{ url('/permissions') }}/"+permission_id)
    	});

		$('#my_search').keyup(function() {
      		dataPermission.search($(this).val()).draw()
		})
	</script>
@endsection
